...
export class NbUserComponent {
   ...
   * @type string
   */
  readonly name = input<string>('Anonymous');
  ...
  readonly imageBackgroundStyle = computed(() => {
    const value = this.picture();
    return value ? this.domSanitizer.bypassSecurityTrustStyle(`url(${value})`) : null;
  });
   ...
   */
  readonly shape = input<NbComponentShape>('round');
  ...
  set showName(val: boolean) {
    this._showName.set(convertToBoolProperty(val));
  }
  private readonly _showName = signal(true);
  ...
  set showTitle(val: boolean) {
    this._showTitle.set(convertToBoolProperty(val));
  }
  private readonly _showTitle = signal(true);
   ...
   * @type boolean
   */
  readonly showInitials = input(true, { transform: convertToBoolProperty });
