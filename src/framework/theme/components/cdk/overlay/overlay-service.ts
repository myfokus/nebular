...
export function patch<T>(container: ComponentRef<T>, containerContext: Object): ComponentRef<T> {
  // Every key passed here must be a signal input on the container component — setInput throws otherwise.
  Object.entries(containerContext).forEach(([key, value]) => container.setInput(key, value));
